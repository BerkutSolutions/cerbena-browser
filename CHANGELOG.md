...
## 1.0.7-2 — повторная live-проверка trusted updater

### Тесты и release gates

- Релиз `1.0.7-2` выпущен поверх установленной `1.0.7-1`, чтобы проверить уже следующий шаг trusted-update цепочки после фикса apply-helper и облегчения startup/rerender path.

### Апдейтер

- Версия `1.0.7-2` не меняет updater-механику относительно `1.0.7-1`; это чистый smoke-check, что установленная `1.0.7-1` корректно находит новый signed release, применяет его и перезапускается уже в новую версию.

## 1.0.7-1 — updater apply-fix и облегчённый старт UI

### Тесты и release gates

- Релиз `1.0.7-1` выпущен сразу после неуспешной живой проверки `1.0.7`, где trusted updater корректно доходил до verified staging, но не переводил установленный launcher на новую версию из-за оставшегося процесса Cerbena в install-root.

### Апдейтер

- ZIP apply-helper теперь не полагается на то, что старый launcher закроется сам: перед заменой файлов он дожидается завершения updater-процесса, принудительно останавливает процессы `cerbena.exe`, `browser-desktop-ui.exe` и `cerbena-updater.exe` из текущего install-root, ждёт освобождения файлов и только потом копирует staged bundle с ретраями и выполняет relaunch.

### Производительность

- Старт desktop UI облегчён: первый bootstrap больше не делает лишний двойной `hydrateProfilesModel`, а чисто визуальные события `locale`, `sidebar` и `resize` теперь не гоняют повторную загрузку profile/network/home/settings-данных.
- Во вкладке `Трафик` локальный refresh, фильтры и poller больше не провоцируют полный глобальный rehydrate всего launcher-состояния, поэтому элементы интерфейса должны отзываться заметно быстрее и без лишней тяжести.

## 1.0.7 — live-проверка обновления после apply/relaunch фикса

### Тесты и release gates

- Релиз `1.0.7` выпущен поверх вручную установленной `1.0.6-2`, чтобы проверить уже полный trusted-update цикл: обнаружение новой версии, secure verification, staging, apply по завершению и перезапуск в новую сборку.

### Апдейтер

- Версия `1.0.7` не меняет саму updater-логику по сравнению с `1.0.6-2`, а служит живой проверкой, что исправленный handoff действительно переводит установленную `1.0.6-2` в новую версию через обычный update flow без ручной переустановки.
