...
4. Для сетевого контура откройте [Маршрутизацию и маршрутный runtime](core-docs/network-routing.md) и [DNS, блоклисты и сервисные фильтры](core-docs/dns-and-filters.md).
...
- Русская ветка пишется полностью на русском, кроме устойчивых технических терминов и имен продуктов вроде `TLS`, `DNS`, `Cloudflare`, `Chromium`, `LibreWolf`, `MCP`.
