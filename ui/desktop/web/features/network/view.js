...
function passwordField(nodeId, value, label, t) {
  return `
    <label>${label}
      <span class="input-icon-field">
        <input data-node-id="${nodeId}" data-node-field="password" type="password" autocomplete="off" value="${escapeHtml(value ?? "")}" />
        <button
          type="button"
          class="input-icon-btn"
          data-action="toggle-password-visibility"
          data-password-visible="false"
          aria-label="${escapeHtml(t("profile.security.showPassword"))}"
          title="${escapeHtml(t("profile.security.showPassword"))}"
        >${eyeOffIcon()}</button>
      </span>
    </label>
  `;
...
function commonEndpointFields(node, t, includeAuth = false) {
  return `
    ...
    ${includeAuth ? `
      ...
      ${passwordField(node.nodeId, node.password, t("network.password"), t)}
    ` : ""}
  `;
}
