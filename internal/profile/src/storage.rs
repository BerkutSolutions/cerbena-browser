...
fn wipe_targets_for_type(root: &Path, ty: WipeDataType, scopes: &[String]) -> Vec<PathBuf> {
...
impl ProfileStorage {
    fn cleanup_cache_dir(&self, cache: &Path) -> Result<CacheCleanupResult, ProfileError> {
        if !cache.exists() {
