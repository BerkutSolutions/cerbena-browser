param(
    ...
    [int]$LocalUpdaterTimeoutMinutes = 4,
    [int]$LocalUpdaterRetryCount = 1
)
...
try {
    ...
    if ((-not $SkipPublishedUpdaterE2E) -and (-not $SkipReleaseBuild)) {
        Step "Updater release artifacts" {
            Invoke-Native "powershell" @(
                "-ExecutionPolicy", "Bypass",
                "-File", "scripts/generate-release-artifacts.ps1"
            ) -Quiet:$CompactOutput
            Invoke-Native "powershell" @(
                "-ExecutionPolicy", "Bypass",
    ...
    if (-not $SkipPublishedUpdaterE2E) {
        Step "Local updater end-to-end test" {
            Invoke-Native "powershell" @(
                "-ExecutionPolicy", "Bypass",
                "-File", "scripts/local-updater-e2e.ps1",
                "-TimeoutMinutes", [string]$LocalUpdaterTimeoutMinutes,
                "-PublishedRetryCount", [string]$LocalUpdaterRetryCount
            ) -Quiet:$CompactOutput
        }
    }
