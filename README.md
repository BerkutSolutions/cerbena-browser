...
`Cerbena Browser` — это настольная платформа защищенного браузинга с zero-trust-подходом, сильной изоляцией профилей, явными стратегиями сетевой изоляции и управляемым launcher/runtime-контуром для `Wayfern` и `LibreWolf`.

Поддержка семейства Firefox в текущем релизе зафиксирована только через `LibreWolf`: устаревший runtime-путь `Camoufox` полностью выведен из продукта, документации и операторских сценариев.

## О продукте

Cerbena Browser — это не надстройка над обычным браузером. Это отдельный launcher и runtime-контур, который управляет:

- изолированными профилями для `Wayfern` и `LibreWolf`;
