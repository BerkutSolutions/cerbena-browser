...
`Cerbena Browser` — это настольная платформа защищенного браузинга с zero-trust-подходом, сильной изоляцией профилей, явными стратегиями сетевой изоляции и управляемым launcher/runtime-контуром для `Chromium`, `ungoogled-chromium` и `LibreWolf`.
...
- изолированными профилями для `Chromium`, `ungoogled-chromium` и `LibreWolf`;
...
- Движки: `Chromium`, `ungoogled-chromium`, `LibreWolf`
- Managed runtime: `sing-box`, `openvpn`, `amneziawg`, `tor`, Docker-managed helper-контейнеры

## Поддержка платформ

- Windows (`x86_64`): основной целевой путь поставки с MSI-first install/update flow.
- Linux (`x86_64`, Debian/Ubuntu class): первый поставочный срез в формате `.deb`.
- Для Linux в текущем срезе обновление через updater не auto-apply: только `manual download`.
- Для Chromium-family на Linux системная песочница зависит от host-настроек AppArmor/userns; при недоступности Cerbena явно показывает диагностику и запускает совместимый fallback.
...
- Linux-хост семейства Debian/Ubuntu, если нужна локальная сборка `.deb` через `npm run build:deb`
- Docker runtime (`Docker Desktop` на Windows или Docker Engine на Linux), если нужен container-backed режим сетевой изоляции
...
Для автоматизированных tagged-релизов используйте GitHub Actions workflow:

- `.github/workflows/release-desktop-bundles.yml` собирает Windows `.msi` и Linux `.deb`,
- подписывает release metadata (`checksums.txt` + `checksums.sig`),
- и прикладывает подписанный набор артефактов к GitHub Release.
...
- `cerbena-browser-<version>.msi` как основной Windows-артефакт install/update handoff;
- `cerbena-browser_<version>_amd64.deb` как опциональный Debian/Ubuntu-пакет, если Linux-срез включен в выпуск;
